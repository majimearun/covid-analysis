...
def get_time_series(url=base_url, save=False):
    ...
    if save:
        ...
        global_ts_df.to_csv("./global_ts.csv", index=False)
        country_confirmed_df.to_csv("./country_confirmed.csv", index=False)
        country_deaths_df.to_csv("./country_deaths.csv", index=False)
        country_recovered_df.to_csv("./country_recovered.csv", index=False)
...
def get_vaccination_update(url=base_url, save=False):
    ...
    if save:
        ...
        vac_country_df.to_csv("./vac_country.csv", index=False)
...
if __name__ == "__main__":
    ...
    _, _ = get_global_update(save=True)
    _ = get_continent_update(save=True)
    _ = get_country_update(save=True)
    _, _, _, _ = get_time_series(save=True)
    _, _ = get_vaccination_update(save=True)
